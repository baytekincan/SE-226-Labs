...
def passGen(user_number):
    password = random.sample(words, user_number)
    return "".join(password)


def rep_with_upper(str):
    random_letter = random.randint(0, len(str))
    newPassword = list(str)
    newPassword[random_letter] = newPassword[random_letter].upper()
    finalPassword = "".join(newPassword)
...
def swap_letters(modified):
    ...
    print("\nFinal version of the password: ", finalPassword)
    print()
    return finalPassword
...
while True:
    ...
    if user_number >= 8 or user_number <= 2:
        print("Invalid value!")
    else:
...
a = passGen(user_number)
b = rep_with_upper(a)

print("\nInitial Password: ", a)
# print("\nRep with upper version: ",b)
c = swap_letters(b)
